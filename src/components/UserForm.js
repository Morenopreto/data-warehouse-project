...
function UserForm({ data }) {
    ...
    const [dataForm, setDataForm] = useState([1]);

    const handleActive = () => {
        Array.from(document.getElementsByTagName('input')).map(item => !!item.value).find(item => item === false) === undefined ? setStatus(false) : setStatus(true);
    }
    const submit = () => {
        // Array.from(document.getElementsByTagName('input')).map(item => setDataForm([...dataForm,item.value]));
        Array.from(document.getElementsByTagName('input')).map(item=> setDataForm([...dataForm,item.value]))
        ...
        <Form className="form-ctn">
            ...
            <Form.Group controlId="formEmailValidation">
                ...
                <Form.Text className="text-muted align-items-center">
                    <label className='label-double-mail'>--------------------------- Ingrese su mail nuevamente ---------------------------</label>
                </Form.Text>
            ...
            <Form.Group controlId="formProfile">
                ...
                <Form.Control as="select" onBlur={() => handleActive()}>
                    <option disabled>User or Admin</option>
                    <option>User</option>
                    <option>Admin</option>
                </Form.Control>
            </Form.Group>
            ...
            <Form.Group controlId="formBasicPassword">
                <Form.Label>Reptir Contrasena</Form.Label>
                <Form.Control type="password" placeholder="Repetir Password" onBlur={() => handleActive()} />
            </Form.Group>

            <Button className='submit-button' variant="primary" disabled={status} onClick={()=>submit()} >
                Submit
    </Button>
        </Form>
