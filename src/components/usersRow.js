import React, { useEffect, useState } from 'react';
import { BiDotsHorizontalRounded } from 'react-icons/bi'
import { Image, ProgressBar } from 'react-bootstrap'
import Acciones from './acciones'
import './css/userRow.css'
let src = 'data:image/jpeg;base64,'


function UsersRows({ data, checkedState }) {

    const [displayAcciones, setDisplayAcciones] = useState(false);
    const [selfChecked, setSelfChecked] = useState(null);
    const { city, company, country, interest, mail, name, position, regions, surname } = data;

    useEffect(() => {
        setSelfChecked(checkedState)
    }, [checkedState]);

    const checkedChange = (prevCheckedState, prevSelfChecked, eventChecked) => {
        console.log(prevCheckedState, prevSelfChecked, eventChecked)
        if (prevCheckedState != selfChecked || prevSelfChecked != eventChecked) {
            setSelfChecked(eventChecked);

        }

    }
    const clickAcciones = () => {
        (displayAcciones) ? setDisplayAcciones(false) : setDisplayAcciones(true);
    }
    return (
        <tr>
            <td ><input type="checkbox" defaultChecked={checkedState} checked={selfChecked} onClick={(e) => checkedChange(checkedState, selfChecked, e.target.checked)} /></td>
            <td className='grid-contacto'><Image className='contacto-img' src={src} roundedCircle /> {`${name} ${surname}`} <label className='contacto-desc label-desc'>{mail}</label></td>
            <td ><span className="grid-pais">{city} <label className='label-desc'>{regions}</label></span></td>
            <td className='td-align'>{company}</td>
            <td className='td-align'>{position}</td>
            <td className='td-align'>
                {interest}</td>
            <td className='td-align porcentaje'><ProgressBar variant="success" label='0%' now={0} /></td>
            <td className='acciones'><BiDotsHorizontalRounded onClick={clickAcciones} /><Acciones displayAcciones={displayAcciones} /></td>
        </tr>

    )
}
export default UsersRows;